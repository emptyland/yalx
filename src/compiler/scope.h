...
namespace yalx {
    ...
    namespace cpl {
        ...
        public:
            ...
            class Keeper {
            public:
                explicit Keeper(T *ns) : ns_(ns) { ns_->Enter(); }
            ...
            [[nodiscard]] virtual std::tuple<Statement *, NamespaceScope *> FindSymbol(std::string_view name) const;

            [[nodiscard]] virtual Statement *FindLocalSymbol(std::string_view name) const;
        ...
        protected:
            explicit NamespaceScope(NamespaceScope **location);
