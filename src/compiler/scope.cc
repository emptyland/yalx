...
namespace yalx::cpl {
    ...
    PackageScope::PackageScope(NamespaceScope **location, Package *pkg, GlobalSymbols *symbols)
            : NamespaceScope(location), pkg_(DCHECK_NOTNULL(pkg)) {
        //Enter();
    ...
    FileUnitScope::FileUnitScope(NamespaceScope **location, FileUnit *file_unit, GlobalSymbols *symobls)
            : NamespaceScope(location), file_unit_(file_unit), symobls_(DCHECK_NOTNULL(symobls)) {
    ...
    DataDefinitionScope::DataDefinitionScope(NamespaceScope **location, IncompletableDefinition *definition)
            : NamespaceScope(location), definition_(DCHECK_NOTNULL(definition)) {
        Enter();
    }
    ...
    int
    DataDefinitionScope::UnimplementMethods(
            std::function<void(InterfaceDefinition *, FunctionDeclaration *)> &&callback) {
        int count = 0;
    ...
    FunctionScope::FunctionScope(NamespaceScope **location, FunctionPrototype *prototype, bool fun_is_reducing)
            : NamespaceScope(location), prototype_(DCHECK_NOTNULL(prototype)), fun_is_reducing_(fun_is_reducing) {
        Enter();
    }
    ...
    BlockScope::BlockScope(NamespaceScope **location, Kind kind, Statement *owns)
            : NamespaceScope(location), kind_(kind), owns_(owns) {
        Enter();
    }
