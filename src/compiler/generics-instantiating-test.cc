...
public:
    ...
    void Instantiate(Statement *ast, std::vector<Type *> argv, Statement **inst,
...
TEST_F(GenericsInstantiatingTest, Sanity) {
    ...
    Type *argv[] = {i32_,};
    auto rs = GenericsInstantiating::Instantiate(nullptr, file_unit->interface(0), &arena_, &feedback_, &resolver,
                                                 1,
                                                 argv, &inst);
    ASSERT_TRUE(rs.ok()) << rs.ToString();
...
TEST_F(GenericsInstantiatingTest, SelfType) {
    ...
    ASSERT_EQ(resolver.symbols["main.Foo<i32>"],
              ifdef->method(0)->prototype()->return_type(0)->AsInterfaceType()->definition());
    ASSERT_EQ(resolver.symbols["main.Foo<i32>"],
              ifdef->method(1)->prototype()->return_type(0)->AsInterfaceType()->definition());
    ASSERT_EQ(resolver.symbols["main.Foo<i64>"],
              ifdef->method(2)->prototype()->return_type(0)->AsInterfaceType()->definition());

    Instantiate(stmt, {f32_}, &inst, &resolver);
    ASSERT_TRUE(nullptr != inst);
    ASSERT_TRUE(inst->IsInterfaceDefinition());
    ifdef = inst->AsInterfaceDefinition();
    ASSERT_EQ(resolver.symbols["main.Foo<f32>"],
              ifdef->method(0)->prototype()->return_type(0)->AsInterfaceType()->definition());
...
TEST_F(GenericsInstantiatingTest, NestedType) {
    ...
    ASSERT_EQ(bar_i32->definition(),
              foo_bar_i32->method(0)->prototype()->return_type(0)->AsInterfaceType()->definition());
...
TEST_F(GenericsInstantiatingTest, RecursiveType) {
    ...
    resolver.symbols["Foo"] = stmt;
    Type *argv[] = {i32_};
    auto rs = GenericsInstantiating::Instantiate(nullptr, stmt, &arena_, &feedback_, &resolver, arraysize(argv),
                                                 argv,
                                                 &inst);
    ASSERT_TRUE(rs.fail());
