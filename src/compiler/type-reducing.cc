...
public:
    SymbolDepsScope(SymbolDepsScope **location, PackageScope *pkg_scope)
            : location_(location), prev_(nullptr), pkg_(DCHECK_NOTNULL(pkg_scope)->pkg()) {
        Enter();
    }
...
public:
    explicit PrimitiveTypesProperties(base::Arena *arena)
            : arena_(DCHECK_NOTNULL(arena)), props_(arena) {}
...
private:
    ...
    void Put(std::string_view owns, std::string_view name, std::initializer_list<Type *> params,
        ...
        auto id = String::New(arena_, name);
        auto fun = new(arena_) FunctionDeclaration(arena_, FunctionDeclaration::kNative, id, proto,
                                                   false/*is_reduce*/,
                                                   SourcePosition::Unknown());
...
private:
    ...
    public:
        explicit InstantiatingResolver(TypeReducingVisitor *owns) : owns_(owns) {}
        ...
        for (auto file: node->source_files()) {
            for (auto var: file->vars()) {
                for (int i = 0; i < var->ItemSize(); i++) {
                    ...
                    if (symbol.IsFound()) {
                        Feedback()->Printf(item->source_position(), "Duplicated symbol: %s",
    ...
    int VisitClassDefinition(ClassDefinition *node) override {
        ...
        if (node->super_calling()) {
            ...
            switch (super_call->callee()->kind()) {
                case Node::kIdentifier:
                    symbol = ResolveIdSymbol("base class", super_call->callee()->AsIdentifier(),
                                             Node::kClassDefinition);
                    break;
                case Node::kDot:
                    symbol = ResolveDotSymbol("base class", super_call->callee()->AsDot(), Node::kClassDefinition);
                    break;
                case Node::kInstantiation:
                    symbol = Instantiate("base class", super_call->callee()->AsInstantiation(),
                                         Node::kClassDefinition);
                    break;
                default:
                    break;
        ...
        for (auto method: node->methods()) {
            ...
            if (method->decoration() == FunctionDeclaration::kOverride) {
                // check override
                auto target = scope.ImplementMethodOnce(method->name()->ToSlice(),
        ...
        if (scope.UnimplementMethods([this, node](auto ift, auto method) {
            Feedback()->Printf(node->source_position(), "Unimplement method: %s::%s%s",
                               ift->name()->data(), method->name()->data(),
                               method->prototype()->signature()->data());
        }) > 0) {
            return -1;
        }
    ...
    int VisitStructDefinition(StructDefinition *node) override {
        ...
        if (node->super_calling()) {
            ...
            switch (super_call->callee()->kind()) {
                case Node::kIdentifier:
                    symbol = ResolveIdSymbol("base struct", super_call->callee()->AsIdentifier(),
                                             Node::kStructDefinition);
                    break;
                case Node::kDot:
                    symbol = ResolveDotSymbol("base struct", super_call->callee()->AsDot(),
                                              Node::kStructDefinition);
                    break;
                case Node::kInstantiation:
                    symbol = Instantiate("base struct", super_call->callee()->AsInstantiation(),
                                         Node::kStructDefinition);
                    break;
                default:
                    break;
    ...
    int VisitDot(Dot *node) override {
        ...
        if (level &&
            !CheckAccessable(down_cast<IncompletableDefinition>(def), field, level, node->source_position())) {
            return -1;
        }
    ...
    int VisitCalling(Calling *node) override {
        ...
        if (!is_others_expr) {
            ...
            if (ast->IsClassDefinition() || ast->IsStructDefinition()) { // Is call constructor
                ...
                if (args.size() != def->parameters_size()) {
                    Feedback()->Printf(node->source_position(),
                                       "Unexpected function number of parameters, %zd vs %zd",
                ...
                for (auto i = 0; i < def->parameters_size(); i++) {
                    ...
                    if (def->parameter(i).field_declaration) {
                        auto item = down_cast<VariableDeclaration::Item>(
                                def->field(def->parameter(i).as_field).declaration->AtItem(0));
                    ...
                    if (!param->Acceptable(args[i], &unlinked)) {
                        Feedback()->Printf(node->source_position(),
                                           "Unexpected constructor parameter[%d] type: `%s', "
                                           "actual is `%s'", i, param->ToString().c_str(),
                ...
                if (def->IsClassDefinition()) {
                    return Returning(
                            new(arena_) ClassType(arena_, def->AsClassDefinition(), def->source_position()));
                } else {
                    return Returning(
                            new(arena_) StructType(arena_, def->AsStructDefinition(), def->source_position()));
                }
            }
            ...
            if (auto val = ast->AsVariableDeclaration(); val && val->owns()->IsEnumDefinition()) {
                ...
                if (args.size() != val->ItemSize()) {
                    Feedback()->Printf(node->source_position(),
                                       "Unexpected enum value number of parameters, %zd vs %zd",
                ...
                for (auto i = 0; i < val->ItemSize(); i++) {
                    ...
                    if (!param->Acceptable(args[i], &unlinked)) {
                        Feedback()->Printf(node->source_position(),
                                           "Unexpected enum value parameter[%d] type: `%s', "
                                           "actual is `%s'", i, param->ToString().c_str(),
                                           args[i]->ToString().c_str());
                        return -1;
                    }
        ...
        if (!type) {
            ...
            if (!type->IsFunctionPrototype()) {
                Feedback()->Printf(node->source_position(), "Invalid type: `%s' for calling",
                                   type->ToString().c_str());
                return -1;
            }
        }
        ...
        for (auto i = 0; i < prototype->params_size(); i++) {
            ...
            if (!param->Acceptable(args[i], &unlinked)) {
                Feedback()->Printf(node->source_position(),
                                   "Unexpected function parameter[%d] type: `%s', actual is `%s'",
    ...
    int VisitArrayInitializer(ArrayInitializer *node) override {
        ...
        if (ar->HasCapacities()) {
            ...
            if (!elem_ty->Acceptable(filling, &unlinked)) {
                Feedback()->Printf(node->source_position(),
                                   "Unexpected array element type: `%s', filling value is `%s'",
        ...
        for (size_t i = 0; i < node->case_clauses_size(); i++) {
            if (auto clause = WhenExpression::ExpectValuesCase::Cast(node->case_clause(i))) {
                ...
                if (!lower_type->IsNumber() && !upper_type->IsNumber()) {
                    Feedback()->Printf(clause->source_position(),
                                       "Between-and case needs number type, actual is `%s',"
                ...
                if (lower_type->primary_type() != upper_type->primary_type()) {
                    Feedback()->Printf(clause->source_position(), "Between-and case bound has different type: `%s',"
                                                                  " `%s'", lower_type->ToString().c_str(),
                                       upper_type->ToString().c_str());
                    return -1;
                }
    ...
    int VisitChannelInitializer(ChannelInitializer *node) override {
        ...
        if (!type->IsIntegral()) {
            Feedback()->Printf(node->capacity()->source_position(),
                               "Incorrect type of channel capacity, need integral,"
    ...
    int VisitTryCatchExpression(TryCatchExpression *node) override {
        ...
        for (auto i = 0; i < node->catch_clauses_size(); i++) {
            ...
            if (auto iter = unique_types.find(matching_type->AsClassType()->definition()); iter !=
                                                                                           unique_types.end()) {
    ...
    int VisitNegative(Negative *node) override {
        ...
        if (!type->IsFloating() && !type->IsSignedIntegral()) {
            Feedback()->Printf(node->source_position(), "Incorrect type of operand, -`%s'",
                               type->ToString().c_str());
            return -1;
        }
        return Returning(type);
    ...
    int VisitSend(Send *node) override {
        ...
        if (ReduceReturningOnlyOne(node->lhs(), &receiver) < 0 ||
            ReduceReturningOnlyOne(node->rhs(), &sendee) < 0) {
            return -1;
        }
...
private:
    ...
    int ReduceBinaryExpression(const char *op, BinaryExpression *expr, OperatingKind kind) {
        ...
        if (!lhs->Acceptable(rhs, &unlinked) || !rhs->Acceptable(lhs, &unlinked)) {
            Feedback()->Printf(expr->source_position(),
                               "Type of operand is not acceptable each other, `%s' <=> `%s'",
        ...
        if (qualified) {
            ...
            if (auto ar = qualified->AsArrayType()) {
                for (auto elem: dimension) {
                    ...
                    if (!init) {
                        Feedback()->Printf(source_position, "Incorrect array initializer dimensons: %d",
                                           dimensions_count);
                        return nullptr;
                    }
                    if (dimensions_count == 1) {
                        dimensions_count = qualified->IsArrayType() ? qualified->AsArrayType()->dimension_count()
                                                                    : 1;
                    ...
                    type = ReduceArrayDimension(init->dimensions(), qualified, dimensions_limit - 1,
                                                dimensions_count,
            ...
            for (auto elem: dimension) {
                ...
                if (auto ar = elem->AsArrayInitializer()) {
                    if (dim_size != 0 && dim_size != ar->dimensions_size()) {
                        Feedback()->Printf(elem->source_position(), "Diffecent dimension size: %zd vs %zd",
                                           dim_size,
                                           ar->dimensions_size());
                        return nullptr;
                    }
                    auto type = ReduceArrayDimension(ar->dimensions(), qualified, dimensions_limit - 1,
                                                     dimensions_count,
                                                     elem->source_position());
    ...
    Type *ReduceType(Type *lhs, Type *rhs) {
        if (lhs->IsNumber() && rhs->IsNumber()) {
            auto ty = static_cast<Type::Primary>(Constants::ReduceNumberType(lhs->primary_type(),
                                                                             rhs->primary_type()));
    ...
    Statement *GetTypeSpecifiedDefinition(Type *type) {
        ...
        case Type::kType_f32:
        case Type::kType_f64:*/ {
                auto symbol = FindGlobal(kLangPackageFullName,
                                         Constants::kPrimitiveTypeClassNames[type->primary_type()]);
...
TypeReducingVisitor::TypeReducingVisitor(Package *entry, base::Arena *arena, SyntaxFeedback *error_feedback)
        : arena_(arena), error_feedback_(error_feedback), props_(arena->Lazy<PrimitiveTypesProperties>()),
          entry_(entry) {
}


base::Status
Compiler::ReducePackageDependenciesType(Package *entry, base::Arena *arena, SyntaxFeedback *error_feedback,
