...
//     + constraint: Type *
    ...
    public:
        ...
        ForeachLoop(Identifier *iterative_destination, IntRange range, Block *body,
                    const SourcePosition &source_position);
        ...
        Expression *iterable() const {
            assert(iteration() == kIterator);
            return iterable_;
        }

        const IntRange &range() const {
            assert(iteration() != kIterator);
            return range_;
        }

        IntRange *mutable_range() {
            assert(iteration() != kIterator);
            return &range_;
        }
    ...
    public:
        ...
        public:
            Field(const String *name, Expression *value, const SourcePosition &source_position)
                    : Node(Node::kMaxKinds, source_position), name_(name), value_or_nested_(true),
                      value_(DCHECK_NOTNULL(value)) {}

            Field(const String *name, Annotation *nested, const SourcePosition &source_position)
                    : Node(Node::kMaxKinds, source_position), name_(name), value_or_nested_(false),
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    template<>
    struct LiteralTraits<const String *> {
...
#define DEFINE_ACTUAL_LITERAL(name, type) \
    ...
    public:
        LambdaLiteral(base::Arena *arena, FunctionPrototype *prototype, Statement *body,
                      const SourcePosition &source_position);
...
//    + operands: Expression *[N]
    ...
    protected:
        inline ExpressionWithOperands(Kind kind, const SourcePosition &source_position)
                : Expression(kind, false, true, source_position), operands_count_(N) {
