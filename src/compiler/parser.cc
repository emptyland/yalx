...
//            return Operators::kAssertedGet;
    ...
    Parser::Parser(base::Arena *arena, SyntaxFeedback *error_feedback)
            : arena_(arena), error_feedback_(error_feedback), lexer_(new Lexer(arena, error_feedback)),
            ...
            switch (token.kind()) {
                ...
                case Token::kAtOutlined: {
                    ...
                    if (Declaration::Is(stmt)) {
                        ...
                        error_feedback_->Printf(anno->source_position(),
                                                "Incorrect annotation declaration, wrong position");
                        *ok = false;
                        return nullptr;
                    }
                    file_unit_->Add(stmt);
                ...
                case Token::kPrivate: {
                    ...
                    if (Declaration::Is(stmt)) {
                        ...
                        error_feedback_->Printf(stmt->source_position(),
                                                "Incorrect access declaration, wrong position");
                        *ok = false;
                        return nullptr;
                    }
                    file_unit_->Add(stmt);
 ...
 */
        ...
        do {
            ...
            if (Test(Token::kLParen)) { // (
                ...
                do {
                    ...
                    auto item = new(arena_) VariableDeclaration::Item(arena_, val, kDashName, ty,
                                                                      ty->source_position());
                ...
                auto item = new(arena_) VariableDeclaration::Item(arena_, val, kDashName, unit,
    ...
    IncompletableDefinition *Parser::ParseIncompletableDefinition(IncompletableDefinition *def,
        ...
        if (Test(Token::kColon)) {
            ...
            if (Test(Token::kLParen)) {
                auto calling = new(arena_) Calling(arena_, callee,
                                                   symbol->source_position().Concat(Peek().source_position()));
                if (!Test(Token::kRParen)) {
                    do {
        ...
        if (Test(Token::kLParen)) {
            do {
                ...
                if (Peek().Is(Token::kIdentifier)) {
                    ...
                    field = new(arena_) Annotation::Field(name, nested,
                                                          field_location.Concat(nested->source_position()));
                } else {
...
//             | expression `<-' expression
    ...
    WhileLoop *Parser::ParseWhileLoop(bool *ok) {
        ...
        auto loop = new(arena_) WhileLoop(nullptr/*init*/, false/*execute_first*/, nullptr/*condition*/,
                                          nullptr/*body*/,
                                          location);
    ...
    UnlessLoop *Parser::ParseUnlessLoop(bool *ok) {
        ...
        auto loop = new(arena_) UnlessLoop(nullptr/*init*/, false/*execute_first*/, nullptr/*condition*/,
                                           nullptr/*body*/,
                                           location);
        ParseConditionLoop(loop, CHECK_OK);
        *loop->mutable_source_position() = location.Concat(loop->body()->source_position());
        ...
        while (!Test(Token::kRBrace)) {
            ...
            if (Test(Token::kIn)) { // `in' expression `..' expression -> statement
                ...
                if (Test(Token::kColon)) { // id `:' type -> statement
                    ...
                    *clause->mutable_source_position() = case_location.Concat(
                            clause->match_values().back()->source_position());
                    case_clause = clause;
                }
            }
...
// finally_clause ::= `finally' block
    ...
    Expression *
    Parser::ParseRemainLambdaLiteral(FunctionPrototype *prototype, const SourcePosition &location, bool *ok) {
    ...
    Type *Parser::ParseAtomType(bool *ok) {
        ...
        switch (Peek().kind()) {
            ...
            case Token::kChan: {
                ...
                location = location.Concat(Peek().source_position());
                Match(Token::kGreater, CHECK_OK);
                type = new(arena_) ChannelType(arena_, ChannelType::kInbility | ChannelType::kOutbility, element,
                                               location);
            }
                break;
            case Token::kLParen: {
