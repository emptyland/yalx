...
TEST_F(TypeReducingTest, Sanity) {
    ...
    auto rs = Compiler::FindAndParseProjectSourceFiles("tests/06-type-reducing-sanity", "libs", &arena_,
                                                       &feedback_,
...

    {
        ...
        rs = Compiler::ReducePackageDependenciesType(main_pkg, &arena_, &feedback_, &symbols);
        ASSERT_TRUE(rs.ok()) << rs.ToString();
...
TEST_F(TypeReducingTest, ClassVars) {
    ...
    base::ArenaVector<Package *> entries(&arena_);
    Package *main_pkg = nullptr;
    auto rs = Compiler::FindAndParseProjectSourceFiles("tests/07-class-var-reducing", "libs", &arena_,
                                                       &feedback_,
                                                       &main_pkg, &entries, &all);
    ASSERT_TRUE(rs.ok()) << rs.ToString();
    std::unordered_map<std::string_view, GlobalSymbol> symbols;
...
TEST_F(TypeReducingTest, WhenExprReducing) {
    ...
    base::ArenaVector<Package *> entries(&arena_);
    Package *main_pkg = nullptr;
    auto rs = Compiler::FindAndParseProjectSourceFiles("tests/15-when-expr-reducing", "libs", &arena_,
                                                       &feedback_,
                                                       &main_pkg, &entries, &all);
    ASSERT_TRUE(rs.ok()) << rs.ToString();
    std::unordered_map<std::string_view, GlobalSymbol> symbols;
...
TEST_F(TypeReducingTest, TryCatchExprReducing) {
    ...
    base::ArenaVector<Package *> entries(&arena_);
    Package *main_pkg = nullptr;
    auto rs = Compiler::FindAndParseProjectSourceFiles("tests/16-try-catch-expr-reducing", "libs", &arena_,
                                                       &feedback_,
                                                       &main_pkg, &entries, &all);
    ASSERT_TRUE(rs.ok()) << rs.ToString();
    std::unordered_map<std::string_view, GlobalSymbol> symbols;
...
TEST_F(TypeReducingTest, SimpleExprReducing) {
    ...
    base::ArenaVector<Package *> entries(&arena_);
    Package *main_pkg = nullptr;
    auto rs = Compiler::FindAndParseProjectSourceFiles("tests/17-simple-expr-reducing", "libs", &arena_,
                                                       &feedback_,
                                                       &main_pkg, &entries, &all);
    ASSERT_TRUE(rs.ok()) << rs.ToString();
    std::unordered_map<std::string_view, GlobalSymbol> symbols;
...
TEST_F(TypeReducingTest, ArrayExprReducing) {
    ...
    base::ArenaVector<Package *> entries(&arena_);
    Package *main_pkg = nullptr;
    auto rs = Compiler::FindAndParseProjectSourceFiles("tests/26-ir-array-init-expr", "libs", &arena_,
                                                       &feedback_,
                                                       &main_pkg, &entries, &all);
    ASSERT_TRUE(rs.ok()) << rs.ToString();
    std::unordered_map<std::string_view, GlobalSymbol> symbols;
