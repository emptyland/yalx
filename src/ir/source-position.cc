...
SourcePositionTable::Scope::Scope(const base::ArenaString *file_name,
...
int SourcePositionTable::FindOrInsertFileName(const base::ArenaString *file_name) {
    auto iter = file_ids_.find(file_name->ToSlice());
    if (iter != file_ids_.end()) {
        return iter->second;
    }
    int spawn_id = static_cast<int>(file_names_.size());
    file_ids_[file_name->ToSlice()] = spawn_id;
